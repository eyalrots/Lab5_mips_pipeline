...
*   **`aux_package.vhd`**: This package file likely contains component declarations for all the VHDL entities defined in the `DUT` directory. This allows them to be instantiated in other VHDL files (like the top-level `MIPS.vhd`).
*   **`cond_comilation_package.vhd`** (Likely "conditional_compilation_package.vhd"): This package defines constants that are used for conditional compilation. This allows the design to be configured for different environments or targets, such as different simulation tools (e.g., ModelSim) or hardware (e.g., specific FPGA memory blocks like M9K or M4K) by changing constant values.
